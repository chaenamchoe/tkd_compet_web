		...
		<div class="col-sm-6 blue" id="youTubePlayer1">
			<div id="video1_info" class="bar" style="color:white">
				...
				<tr>
					...
					<?php }else{ ?>
						<td width="auto"><h1 class="name"><b>&nbsp; <?=$a_name[0]?></b></h1></td>
					<?php } ?>
				</tr>
				<tr>
					<?php if($_SESSION['is_international']==1 && isset($country[1])){ ?>
			...
			<?php if($unattend[0] == 0 && (strlen($video_id[1]) > 5)){ ?>
			<iframe id="video1" src="https://player.vimeo.com/video/<?=$video_id[0]?>?autoplay=1&autopause=0"  width="100%" height="100%"
			...
			<div id="video2_info" class="bar" style="color:white">
				...
				<tr>
					<?php if($_SESSION['is_international']==1 && isset($country[1])){ ?>
						...
						<td><h1 class="name"><b>&nbsp; <?=$a_name[1]?></b></h1></td>
					<?php }else{ ?>
						<td><h1 class="name"><b>&nbsp; <?=$a_name[1]?></b></h1></td>
					<?php } ?>	
				</tr>
				<tr>
					...
					<?php }else{ ?>
					<td><h3 class="center">&nbsp; <?=$c_name[1]?></h3></td>
					<?php } ?>
				</tr>
				<tr>
					<td id="timer2"><h1 class="name">0:00</h1></td>
				</tr>
				</table>
			</div>
			<?php if($unattend[1] == 0 && (strlen($video_id[1]) > 5)){ ?>
			<iframe id="video2" src="https://player.vimeo.com/video/<?=$video_id[1]?>?autoplay=1&autopause=0"  width="100%" height="100%"
				frameborder="0" allow="autoplay; fullscreen" allowfullscreen>
			</iframe>
			<?php }else{ ?>
			<div id="video2_info" class="bar" style="color:white">

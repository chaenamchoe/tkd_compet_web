...
@media (min-width: 1200px) {  
  ...
  h2 {font-size:2.5rem;} /*1rem = 16px*/    
  h3 {font-size:2.5rem;} /*1rem = 16px*/    
  img.ath-picture {width:120px; height:100px;}
  img.fit-picture{width:120px; height:100px;}
}
  ...
  <div id="text" style="margin:auto">Overlay Text</div>
	...
	<?php 
		...
		foreach ($lists as $row){
			...
			$a_name = $row['A_NAME'];
			$c_name = $row['A_CENTER'];
			$j1point = $row['J1_POINT'];
			$j2point = $row['J2_POINT'];
			$j3point = $row['J3_POINT'];
			$j4point = $row['J4_POINT'];
			$j5point = $row['J5_POINT'];
			$min_idx = $row['MIN_IDX2'];
			$max_idx = $row['MAX_IDX2'];
			$judge_cnt = 5;
			//$totals = $row['TXT_TOTAL'] == "0.00" ? "" : $row['TXT_TOTAL'];
			$totals = number_format($row['POINT_TOTAL'], 2, '.', ' ');
			$failed = $row['FAILED'];
			...
			if($_SESSION['is_international']==1){
			...
			if(isset($ath_data->A_PICTURE) && strlen($ath_data->A_PICTURE) > 10){
		...
		<div class="player">
			...
			<div class="picture">
				<?php if($_SESSION['use_athlete_picture']==1){
					if((strpos(strtolower($picture_id), '.jpg') !== false) ||
			...
			<div class="totals text-center">
				...
				<table>
				<tr>
					<?php if($min_idx==1 or $max_idx==1){ ?>
					<td class="total" style="color:green"><h3><b><?=number_format($j1point, 2, '.', ' ');?></b></h3></td>
					<?php }else{ ?> 
					<td class="total"><h3><b><?=number_format($j1point, 2, '.', ' ');?></b></h3></td>
					<?php } ?>
					<td class="total"><h3>&nbsp</h3></td>
					<?php if($min_idx==2 or $max_idx==2){ ?>
					<td class="total" style="color:green"><h3><b><?=number_format($j2point, 2, '.', ' ');?></b></h3></td>
					<?php }else{ ?> 
					<td class="total"><h3><b><?=number_format($j2point, 2, '.', ' ');?></b></h3></td>
					<?php } ?>
					<td class="total"><h3>&nbsp;</h3></td>

					<?php if($min_idx==3 or $max_idx==3){ ?>
					<td class="total" style="color:green"><h3><b><?=number_format($j3point, 2, '.', ' ');?></b></h3></td>
					<?php }else{ ?> 
					<td class="total"><h3><b><?=number_format($j3point, 2, '.', ' ');?></b></h3></td>
					<?php } ?>
				<?php if($judge_cnt == 5){ ?>
					<td class="total"><h3>&nbsp;</h3></td>
					<?php if($min_idx==4 or $max_idx==4){ ?>
					<td class="total" style="color:green"><h3><b><?=number_format($j4point, 2, '.', ' ');?></b></h3></td>
					<?php }else{ ?> 
					<td class="total"><h3><b><?=number_format($j4point, 2, '.', ' ');?></b></h3></td>
					<?php } ?>
					<td class="total"><h3>&nbsp;</h3></td>

					<?php if($min_idx==5 or $max_idx==5){ ?>
					<td class="total" style="color:green"><h3><b><?=number_format($j5point, 2, '.', ' ');?></b></h3></td>
					<?php }else{ ?> 
					<td class="total"><h3><b><?=number_format($j5point, 2, '.', ' ');?></b></h3></td>
					<?php } ?>
				<?php } ?>	
				</tr>
				<tr>
				<?php if($judge_cnt == 5){ ?>
					<td colspan="8"><td style="color:orange" class="total"><h3><b><?=$totals?></b></h3></td>
				<?php }else{ ?>	
					<td colspan="4"><td style="color:orange" class="total"><h3><b><?=$totals?></b></h3></td>
				<?php } ?>	
				</tr>
				<!--<h4 class="total"><span><?=$points?></span></h4><h3><span style="color:orange"><b><?=$totals?></b></span></h3>-->
				</table>
				<?php } ?>
			</div>
			<div class="winkind">

			...
			<?php if($judge){
					...
					foreach ($judge as $row){
						...
						?>
						  ...
						  <?php } ?>	
						  <div class="card-header text-center" style="background-color:#003c8f; color: white;"><h3>Judge <?=$i?></h3></div>
  						  <?php if($show_judge_picture == 1){ ?>
						  <img class="fit-picture" src="http://ccnplaza.com/tkd_compet/judge/<?=$j_id.'.jpg' ?>" width="100%" height="380px"> 
						  <?php } ?>
						  <div class="card-body">
							<h5 class="card-title text-center" style="color:#003c8f;"><b><?=$j_name?></b></h5>
						  </div>
						</div>
				<?php } ?>
			<?php } ?>

  ...
  <br/>  
	...
	<div class="row justify-content-center">
		...
		<form id="login-form" action=<?php echo site_url("compet/check_login")?> method="post">
			<div class="card-header bg-primary text-white"><h5 class="card-title text-center">사용자 로그인</h5></div>	
			<div class="card-body">
				<p class="text-center" style="color:red; font-weight:bold">등록한 이메일과 비밀번호로 로그인하세요.<br>
					비등록 도장(단체)은 신규등록을 먼저 하세요.</p>
				<?php if($_SESSION['assoc_id'] == 0){ ?>
				<p>
					<label>소속협회를 선택하세요.</label>
					<select class="form-control" name="assoc_id" id="assoc_id" required>
					<?php if(count($result) > 1){
						echo "<option value=0>--소속협회를 선택하세요--</option>";

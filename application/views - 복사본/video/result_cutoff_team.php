...
<div class="container-fluid">
	...
	<?php 
		...
		foreach ($lists as $row){
			...
			$point1 = number_format($row['J1_POINT'], 2);
			$point2 = number_format($row['J2_POINT'], 2);
			$point3 = number_format($row['J3_POINT'], 2);
			$point4 = number_format($row['J4_POINT'], 2);
			$point5 = number_format($row['J5_POINT'], 2);
			...
			if($game_kind==1){
				$win_kind = $row['TXT_TOTAL'] == "0.00" ? "" : $row['WIN_KIND'].'À§';
			}else{
				if($row['WIN_KIND']==1){
					$win_kind = $row['TXT_TOTAL'] == "0.00" ? "" : 'WIN';
				}else{
					$win_kind = '';
				}
			}
		...
		<div class="col-md" id="youTubePlayer1">
			<div id="video1_info" class="text-center">
				...
				<?php if($_SESSION['judge_count'] == 1){ ?>
					...
					<img class="ath-picture img-thumbnail" width="240px" height="320px" src="http://ccnplaza.com/tkd_compet/uploads/<?=$picture_id?>">
					<?php } ?>
					<?php if($point1 + $point2 + $point3 > 0){ ?>
					<h1 class="total"><b><?=$totals?></b><br>
					<!--<h3 class="points" style="font-size:400%"><?=$points?></h3>-->
						<span style="color:yellow"><?=$point1?>&nbsp;</span>
						<span style="color:yellow"><?=$point2?>&nbsp;</span>
						<span style="color:yellow"><?=$point3?>&nbsp;</span>
					</h1>
					<?php } ?>
				<?php }else{ ?>
					<?php if($_SESSION['is_international']==1){ ?>
					...
					<?php } ?>
					<?php if($point1 + $point2 + $point3 + $point4 + $point5 > 0){ ?>
					<h1 class="total"><b><?=$totals?></b><br>
					<!--<h3 class="points" style="font-size:400%"><?=$points?></h3>-->
					<?php if($min_point==1 or $max_point==1){ ?>
						<span style="color:red; text-decoration: line-through;"><?=$point1?>&nbsp;</span>
					<?php }else{ ?>
						<span style="color:yellow"><?=$point1?>&nbsp;</span>
					<?php } ?>

					<?php if($min_point==2 or $max_point==2){ ?>
						<span style="color:red; text-decoration: line-through;"><?=$point2?>&nbsp;</span>
					<?php }else{ ?>
						<span style="color:yellow"><?=$point2?>&nbsp;</span>
					<?php } ?>

					<?php if($min_point==3 or $max_point==3){ ?>
						<span style="color:red; text-decoration: line-through;"><?=$point3?>&nbsp;</span>
					<?php }else{ ?>
						<span style="color:yellow"><?=$point3?>&nbsp;</span>
					<?php } ?>

					<?php if($min_point==4 or $max_point==4){ ?>
						<span style="color:red; text-decoration: line-through;"><?=$point4?>&nbsp;</span>
					<?php }else{ ?>
						<span style="color:yellow"><?=$point4?>&nbsp;</span>
					<?php } ?>

					<?php if($min_point==5 or $max_point==5){ ?>
						<span style="color:red; text-decoration: line-through;"><?=$point5?>&nbsp;</span>
					<?php }else{ ?>
						<span style="color:yellow"><?=$point5?>&nbsp;</span>
					<?php }} ?>
					</h1>
				<?php } ?>	
			</div>
	  ...
	  <div class="modal-dialog modal-lg modal-dialog-centered">
		<div class="modal-content">
		  <div class="modal-header">
			<h4 class="modal-title" id="exampleModalLabel">¾Ë¸²</h4>
